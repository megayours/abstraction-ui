...
export interface FilterFormProps {
  onSubmit: (filter: AssetFilter) => void;
  onEdit?: (filter: AssetFilter) => void;
...
export function FilterForm({ onSubmit, onEdit, initialValues }: FilterFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSource || !selectedAsset) {
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" disabled={!selectedSource || !selectedAsset}>
        {onEdit ? 'Update Filter' : 'Add Filter'}
      </Button>
    </form>
  );
} 